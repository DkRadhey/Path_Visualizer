...
function clicked(elementID) {
    ...
    else node.style.backgroundColor = original;
}
...
function bfs() {
    reset1();
    var maze = [];
    for (let i = 0; i < len; i++) maze[i] = new Array(len).fill(0);
    for (let row = 0; row < len; row++) {
        for (let col = 0; col < len; col++) {
            if (
                document.getElementById("node" + (row * len + (col + 1))).style
    ...
    var visited = [];
    for (let i = 0; i < len; i++) visited[i] = new Array(len).fill(0);
    visited[0][0] = 1;

    var pred = [];
    for (let i = 0; i < len; i++) pred[i] = new Array(len).fill(0);
    pred[0][0] = -1;
    ...
    while (queue.length > 0) {
        ...
        if (
            u[0][0] - 1 >= 0 &&
            maze[u[0][0] - 1][u[0][1]] == 0 &&
            visited[u[0][0] - 1][u[0][1]] == 0
        ) {
        ...
        if (
            u[0][1] - 1 >= 0 &&
            maze[u[0][0]][u[0][1] - 1] == 0 &&
            visited[u[0][0]][u[0][1] - 1] == 0
        ) {
...
function dfs1(maze, visited, sx, sy, path) {
    ...
    if (visited[sx][sy] == 0) {
        ...
        if (sx + 1 < len && visited[sx + 1][sy] == 0 && maze[sx + 1][sy] == 0) {
            path.push([sx + 1, sy]);
            if (dfs1(maze, visited, sx + 1, sy, path)) return 1;
            path.pop();
        }
        ...
        if (sy + 1 < len && visited[sx][sy + 1] == 0 && maze[sx][sy + 1] == 0) {
            path.push([sx, sy + 1]);
            if (dfs1(maze, visited, sx, sy + 1, path)) return 1;
            path.pop();
        }
...
function bdbfs1(pred1, pred2, maze) {
    ...
    var visited1 = [];
    for (let i = 0; i < len; i++) visited1[i] = new Array(len).fill(0);
    visited1[0][0] = 1;

    var visited2 = [];
    for (let i = 0; i < len; i++) visited2[i] = new Array(len).fill(0);
    ...
    while (q1.length > 0 && q2.length > 0) {
        ...
        if (
            u1[0][0] - 1 >= 0 &&
            visited1[u1[0][0] - 1][u1[0][1]] == 0 &&
            maze[u1[0][0] - 1][u1[0][1]] != -1
        ) {
        ...
        if (
            u2[0][0] - 1 >= 0 &&
            visited2[u2[0][0] - 1][u2[0][1]] == 0 &&
            maze[u2[0][0] - 1][u2[0][1]] != -1
        ) {
        ...
        if (
            u1[0][1] - 1 >= 0 &&
            visited1[u1[0][0]][u1[0][1] - 1] == 0 &&
            maze[u1[0][0]][u1[0][1] - 1] != -1
        ) {
        ...
        if (
            u2[0][1] - 1 >= 0 &&
            visited2[u2[0][0]][u2[0][1] - 1] == 0 &&
            maze[u2[0][0]][u2[0][1] - 1] != -1
        ) {
...
function bdbfs() {
    ...
    var pred1 = [];
    for (let i = 0; i < len; i++) pred1[i] = new Array(len).fill(0);
    pred1[0][0] = -1;

    var pred2 = [];
    for (let i = 0; i < len; i++) pred2[i] = new Array(len).fill(0);
    pred2[len - 1][len - 1] = -1;
